...
class MlxLlmTranslationSimul(MlxLlmTranslation):
    ...
    def __init__(
        ...
        super().__init__(
            model_id=model_id, target_language=target_language, warmup=warmup
        )
    ...
    def _translate_simul(self, source_text: str, committed_text: str) -> str:
        ...
        content = self._config.prompt_template.format(
            target_lang=self._target_name, text=source_text
        )
    ...
    def _release_held(self, committed_text: str) -> str:
        ...
        content = self._config.prompt_template.format(
            target_lang=self._target_name, text=self._last_source_text
        )
        prompt_str = tokenizer.apply_chat_template(
    ...
    def process(self) -> Tuple[Optional[Translation], TimedText]:
        ...
        if source == self._last_source_text and self._last_draft is not None:
            # Source unchanged: the ASR committed more of the tail but the
            # total source is the same → release held tokens without a call.
            released = self._release_held(committed)
            if released and not released.startswith(self._emitted_partial):
                # New draft prefix (rare on a pure release); keep the longer.
                self._emitted_partial = released
            elif released and len(released) > len(self._emitted_partial):
                self._emitted_partial = released
        else:
