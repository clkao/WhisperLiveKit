...
# Hunyuan-MT prompt from the model README. The backend reads it from the
# config dict, not as a hardcoded class attribute.
HUNYUAN_MT_PROMPT = "把下面的文本翻译成{target_lang}，不要额外解释。\n\n{text}"

# Full language names for the prompt. Decoder-LLM MT prompts expect language
# names, not ISO codes.
_TARGET_LANG_NAME = {
    "en": "English",
    "zh": "中文",
...
class MlxLlmMtModelConfig:
    """Per-model config for the mlx-llm MT backend.

    A model differs from another only in these values: the HF repo, the
    prompt template, the EOS token, and sampling params. Add a model by
    adding a config-dict entry, not by writing code.
    """
...
MTX_MODEL_CONFIGS: Dict[str, MlxLlmMtModelConfig] = {
    ...
    # A different repo + prompt loads without new code.
