...
class Qwen3ASRThinkerTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3ASRConfig, device=None):
        ...
        self.config = config
        # transformers 5.x removed the "default" entry from ROPE_INIT_FUNCTIONS
        # (plain RoPE, no scaling) and moved config from `rope_scaling` to
        # `rope_parameters`. The text tower uses unscaled RoPE, so fall back to the
        # original inv_freq formula when the rope_type isn't in the registry.
