...
## Stage Report: implementation (refactor)

_Refactored PR1 for coherence: cut the hunyuan-mlx alias, extracted model profiles, wired the benchmark translation path, fixed the 500% WER, and ran the en→it comparison._

- DONE: Cut the hunyuan-mlx alias and shim subclass. Deleted `translation_hunyuan_mlx.py`; removed `hunyuan-mlx` from `--translation-backend` choices (kept only `nllb`, `alignatt`, `mlx-llm-mt`); removed `--hunyuan-mlx-model` flag and `hunyuan_mlx_model` config field with its `__post_init__` reconciliation; removed `hunyuan-mlx` from `core.py` dispatch. The only translation-backend name for this path is `mlx-llm-mt`; the only model flag is `--mlx-llm-mt-model`.
  Verified: `grep -rn 'hunyuan.mlx\|HunyuanMlx\|hunyuan_mlx\|hunyuan-mlx' --include='*.py'` returns empty (excluding venv/third_party).
- DONE: Moved Hunyuan-specific configs to a separate profiles module. Created `translation_mlx_llm_mt_profiles.py` with the six Hunyuan-MT entries + TranslateGemma entry. The base module (`translation_mlx_llm_mt.py`) keeps only `MlxLlmMtModelConfig`, `MlxLlmTranslation`, `_ensure_model`, the 5-method contract, warmup, `_translate_text`, `_mt_call_count`, `_mt_total_time_s`, and an empty `MTX_MODEL_CONFIGS` registry populated by the profiles module at import time. No model-specific code or names remain in the base module.
  Verified: `grep -in 'hunyuan\|_HY\b' translation_mlx_llm_mt.py` returns only the docstring mention (line 3); the profiles module has 7 config entries (6 Hunyuan + 1 TranslateGemma).
- DONE: Fixed the 500% WER. The `zh_long` benchmark sample's reference is set to empty (its audio content does not match any known transcript). The runner skips WER computation when the reference is empty and reports N/A instead of 500%. Added `wer_applicable` flag to `SampleResult` and display logic in the report.
  Verified: runner.py lines 148-162 check `sample.reference and sample.reference.strip()` before computing WER; report.py shows "N/A" when `wer_applicable` is False.
- DONE: Added en→it example run; compared mlx-llm-mt vs NLLB. Ran `wlk bench --backend mlx-qwen3-asr --translation-backend mlx-llm-mt --target-language it --languages en` and `--translation-backend nllb`. Both produce correct Italian translations. mlx-llm-mt: MT-RTF 0.58x, first-translation 17.0s, 17 MT calls. NLLB: first-translation 33.4s, no MT-RTF instrumentation. Reports saved to `_work/pr1_en_it_mlx_llm_mt.txt` and `_work/pr1_en_it_nllb.txt`.
  Verified: both report files contain correct Italian translation output; mlx-llm-mt shows faster first-translation (17s vs 33s).
- DONE: Wired the benchmark translation path into `wlk bench`. Added `--translation-backend`, `--target-language`, `--simultaneous`, `--reference-translation` flags; the runner threads them to TestHarness, tracks translation timing, extracts MT-call count and wall-time, renders a translation table. Registered `mlx-qwen3-asr` in the benchmark compat matrix. Added the `mlx-qwen3-asr` ASR backend dispatch in `core.py` (config fields, `_do_init` branch, `online_factory` routing).
  Verified: `wlk bench --help` shows all translation flags; benchmark runs successfully with `--backend mlx-qwen3-asr --translation-backend mlx-llm-mt --target-language it`.
- DONE: All 11 tests pass after the refactor. Added `test_registry_populated_by_profiles_module` verifying all six Hunyuan entries + TranslateGemma are present and the Hunyuan prompt lives in the config (not a module-level constant). Removed `test_hunyuan_mlx_reexports` (the shim file is deleted).
  Verified: `.venv/bin/python -m pytest tests/test_mlx_llm_mt.py -q` → 11 passed.
- DONE: PR description updated at `_work/pr1_description.md` — no hunyuan-mlx alias, no backward-compat, profiles module, en→it comparison table.
  Verified: description mentions profiles module, en→it comparison, and states "no backward-compat".

### Summary (refactor)

The refactor cut all hunyuan-mlx backward-compat (alias, shim, config field, flag), extracted model configs into a profiles module so the base backend has zero model-specific code, wired the benchmark translation path into `wlk bench` with translation metrics, fixed the 500% WER by skipping WER for empty references, and ran a live en→it comparison between mlx-llm-mt (Hunyuan-MT) and NLLB showing both produce correct Italian translations with mlx-llm-mt having faster first-translation latency. The diff is 13 files across two commits (5c4f223, 666bddd) on top of the initial implementation, with all 11 tests passing.
